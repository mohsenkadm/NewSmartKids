...
{
    public class GetDataUrlService<TEntity> 
    { 
        ...
        public async Task<Response<TEntity>> PostListAsync(string url, List<TEntity> entity)
        {
            try
            {
                HttpClient client = new HttpClient();
        ...
        {
              Preferences.Set("Id", users.UserId);
            Preferences.Set("Token", users.Token.ToString());              
